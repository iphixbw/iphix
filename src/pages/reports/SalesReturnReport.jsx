...
export default function SalesReturnReport({ shops, onBack }) {
  ...
  async function fetchData() {
    ...
    let query = supabase
      .from('sales_returns')
      .select('*, customers(name, customer_no), salesmen(name), shops(name)')
  ...
  function exportCSV() {
    const headers = ['Return No', 'Date', 'Customer', 'Customer No', 'Salesman', 'Shop', 'Total', 'Payment', 'Status', 'Remarks']
    const rows = filtered.map(r => [r.return_no, new Date(r.created_at).toLocaleDateString('en-GB'), r.customers?.name, r.customers?.customer_no, r.salesmen?.name, r.shops?.name, r.total, r.payment_method, r.status, r.remarks])
    const csv = [headers, ...rows].map(r => r.map(v => `"${v || ''}"`).join(',')).join('\n')
    const blob = new Blob([csv], { type: 'text/csv' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = `sales-returns-${dateFrom}-${dateTo}.csv`; a.click()
  ...
  function printReport() {
    ...
    const rows = filtered.map((r,i) => `<tr>
      ...
      <td><strong>${r.customers?.name||'—'}</strong><br><span style="color:#94a3b8;font-size:9px">${r.customers?.customer_no||''}</span></td>
      <td>${r.salesmen?.name||'—'}</td>
      <td>${r.shops?.name||'—'}</td>
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>Sales Returns Report</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;font-size:11px;color:#1e293b;padding:16px}.hdr{border-bottom:3px solid #1e40af;padding-bottom:10px;margin-bottom:14px}.co{font-size:19px;font-weight:900;color:#1e40af;letter-spacing:1px}.ttl{font-size:14px;font-weight:700;color:#0f172a;margin-top:3px}.sub{font-size:10px;color:#64748b;margin-top:2px}.summary{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;margin-bottom:14px}.card{border:1.5px solid #e2e8f0;border-radius:8px;padding:10px 14px}.lbl{font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px}.val{font-size:15px;font-weight:800}table{width:100%;border-collapse:collapse;font-size:10px}thead tr{background:#0f172a;color:white}thead th{padding:7px 8px;font-size:9px;font-weight:700;text-transform:uppercase;white-space:nowrap;text-align:left}thead th.r{text-align:right}thead th.c{text-align:center}tbody tr:nth-child(even){background:#f8fafc}tbody td{padding:5px 8px;border-bottom:1px solid #f1f5f9;vertical-align:middle}tbody td.r{text-align:right}tbody td.c{text-align:center}tfoot tr{background:#1e293b;color:white}tfoot td{padding:8px;font-weight:800;font-size:11px}tfoot td.r{text-align:right}.ftr{margin-top:14px;border-top:2px solid #e2e8f0;padding-top:8px;text-align:center;font-size:9px;color:#94a3b8;font-weight:700}@media print{@page{size:A4 landscape;margin:8mm}body{padding:0}}</style></head><body>
      ...
      <div class="summary">
        ...
        <div class="card"><div class="lbl">Cash Refunds</div><div class="val" style="color:#059669">LKR ${fmt(byCash)}</div></div>
        <div class="card"><div class="lbl">Credit Deducted</div><div class="val" style="color:#d97706">LKR ${fmt(byCredit)}</div></div>
      </div>
      <table><thead><tr><th>Return No</th><th>Date</th><th>Customer</th><th>Salesman</th><th>Shop</th><th class="r">Amount</th><th>Payment</th><th class="c">Status</th><th>Remarks</th></tr></thead>
      <tbody>${rows}</tbody>
      <tfoot><tr><td colspan="5">TOTALS — ${filtered.length} returns</td><td class="r" style="color:#fca5a5">LKR ${fmt(totalValue)}</td><td colspan="3"></td></tr></tfoot>
      </table>
    ...
    <div>
      ...
      <div className="no-print" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          ...
          <h1 style={{ fontSize: '22px', fontWeight: '700', color: '#0f172a', margin: 0 }}>Sales Returns Report</h1>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '16px', marginBottom: '20px' }}>
        {[
          ...
          { label: 'Cash Refunds', value: formatCurrency(byCash), color: '#059669' },
          { label: 'Credit Deducted', value: formatCurrency(byCredit), color: '#d97706' },
        ].map(s => (
      ...
      <div className="no-print" style={{ marginBottom: '12px' }}>
        <input type="text" placeholder="Search return no or customer…" value={search} onChange={e => setSearch(e.target.value)} style={{ ...inp, width: '100%', boxSizing: 'border-box' }} />
      </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '900px' }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                  {['Return No', 'Date', 'Customer', 'Salesman', 'Shop', 'Total', 'Payment', 'Status', 'Remarks'].map(h => (
                ...
                {filtered.map((r, i) => (
                  <tr key={r.id} style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 === 0 ? 'white' : '#fafafa' }}>
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <div style={{ fontSize: '13px', fontWeight: '600', color: '#0f172a' }}>{r.customers?.name}</div>
                      <div style={{ fontSize: '11px', color: '#94a3b8' }}>{r.customers?.customer_no}</div>
                    </td>
                    <td style={{ padding: '10px 14px', fontSize: '13px', color: '#64748b' }}>{r.salesmen?.name || '—'}</td>
