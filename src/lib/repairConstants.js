...
export function printJobReceipt(job, customer) {
...
export function printJobPaymentReceipt(job, customer, paymentAmount, paymentMethod) {
  const w = window.open('', '_blank')
  const fmt2 = n => (parseFloat(n) || 0).toLocaleString('en-LK', { minimumFractionDigits: 2 })
...
// ── Parts sale receipt ──────────────────────────────────
export function printPartsSaleReceipt(sale, items, customerName) {
  const w = window.open('', '_blank')
  const fmt2 = n => (parseFloat(n) || 0).toLocaleString('en-LK', { minimumFractionDigits: 2 })
